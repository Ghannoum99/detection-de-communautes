...
t_matrice_adjascence* allouerMatriceAdjascence(int nbr_sommets)
{
	...
	mat->nombre_lignes_colonnes = nbr_sommets;
...
void desallouerMatriceAdjascence(t_matrice_adjascence** matrice_adjascence)
{
	...
	for(i=0; i<(*matrice_adjascence)->nombre_lignes_colonnes ; i++)
	{
...
{	
	saisirValeurs(mat->coeffs,mat->nombre_lignes_colonnes);
}
...
void genererGrapheAleatoire()
{
	...
	saisirMatrice(graphe_aleatoire.matrice_d_adjascence);
	...
	desallouerMatriceAdjascence(&graphe_aleatoire.matrice_d_adjascence);
}
